...
const MenuPage: React.FC = () => {
  ...
  const menuItems = [
    { label: 'Deposit', icon: Plus, action: () => navigate('/wallet'), highlight: true },
        ...
        <div className="space-y-4">
            {menuItems.map((item, index) => {
                ...
                return (
                    ...
                    >
                        <span className={`text-4xl font-display font-bold tracking-tight transition-colors ${item.highlight ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'}`}>
                            {item.label}
                        </span>
                        <div className={`p-3 rounded-full transition-transform group-hover:rotate-12 ${item.highlight ? 'bg-orby text-white shadow-[0_0_20px_rgba(40,80,255,0.4)]' : 'bg-zinc-900 text-zinc-500 group-hover:text-zinc-300'}`}>
