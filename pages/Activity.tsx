    ...
    <Layout bgClass="bg-orby">
      <div className="flex flex-col text-white px-6 pt-6 pb-32 min-h-full">
        ...
        <div className="flex flex-col gap-4">
            <h2 className="font-display font-bold text-2xl mb-2 opacity-80 tracking-tight animate-fade-in text-white">Recent Activity</h2>
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className={`p-3 rounded-full transition-transform duration-300 group-hover:scale-110 shadow-sm ${isPositive ? 'bg-white text-orby' : 'bg-black/20 text-white'}`}>
                                    <Icon size={20} strokeWidth={2.5} />
                                </div>
                                <div>
                                    <h3 className="font-display font-bold text-lg leading-tight tracking-tight text-white">{tx.title}</h3>
                                    <p className="text-xs font-semibold opacity-60 uppercase tracking-wide font-sans text-white">{tx.time}</p>
                                </div>
                            </div>
                            <span className={`text-xl font-bold tracking-tight font-sans ${isPositive ? 'text-white' : 'text-white'}`}>
                                {tx.amount}
                            </span>
                        </div>
                    );
