...
import { UserBalance, FestivalConfig } from '../types';
...
const WalletPage: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const bal = await api.getBalances("user123", "sessionToken");
      const fest = await api.getFestival();
      setBalance(bal);
      setFestival(fest);
    } catch (e) {
      console.error(e);
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      toast.className = "fixed top-6 left-1/2 transform -translate-x-1/2 bg-black/90 backdrop-blur-md text-white px-6 py-4 rounded-2xl border border-white/10 shadow-2xl z-50 font-bold animate-fade-in-down font-display tracking-tight flex items-center gap-3";
      toast.innerHTML = `<span class="text-orby">✓</span> Top up successful!`;
  ...
  // Convert 18 decimals to float for display
  const displayTokenBalance = balance 
    ? (parseFloat(balance.festivalTokens) / 1e18).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })
    : "0";
  
  const displayUsdtBalance = balance
            ...
            <div className="text-center transform -translate-y-8 animate-scale-in">
                ...
                <div className="flex flex-col gap-2 items-center opacity-90 font-medium text-lg font-sans animate-fade-in-up delay-100">
                    {loading ? (
                        ...
                        <>
                            <span className="tracking-widest uppercase text-xs font-bold text-white/80 mb-1">{festival ? festival.festivalSymbol : 'TOKENS'} Available</span>
